<template>
    <table :class="tableClass" role="table">
      <!-- table header -->
      <thead role="rowgroup">
        <th v-for="c in tColumns" :key="c.key" role="columnheader" :class="getColumnClass(c)">
          <div @click="sortByCol(c)">
            {{ c.label }}
            <!-- select the right sorting icon -->
            <i
              v-if="
                c.sortable == true && sortCol != null && c.label == sortCol.label && sortOrder == 0
              "
              class="bi bi-caret-down-fill"
            ></i>
            <i
              v-if="
                c.sortable == true && sortCol != null && c.label == sortCol.label && sortOrder == 1
              "
      ...
      <tbody v-if="tRows.length > 0" role="rowgroup">
        ...
        >
          <td v-for="c in Array(tColumns.length).keys()" :key="c" :class="getColumnClass(tColumns[c ])">
...
export default {
  ...
  computed: {
    tableClass: function () {
      if (this.customTableClass != null) {
        return "table " + this.getTableSizing() + " table-hover table-flex" + this.customTableClass;
      }
  ...
  methods: {
    ...
    sortByCol: function (col, changeSorting=true) {
      if (changeSorting && this.sortCol != null && this.sortCol.label == col.label) {
