...
export default class BooksysPie {
  static addPiePlugin = () => {
    Raphael.fn.pieChart = function (
      ...
      this.selectSector = function (i) {
        ...
        sectors[i]
          .stop()
          .animate({ transform: "s1.15 1.15 " + cx + " " + cy }, 0, "elastic");
        sector_active = 1;
        sectors_active[i] = 1;
      };
  ...
  drawPie(location, data, callback, properties) {
    ...
    const dayStart = dayjs
      .unix(Math.max(sunrise.format("X"), business_day_start.format("X")))
      .format();
    const dayEnd = dayjs
      .unix(Math.min(sunset.format("X"), business_day_end.format("X")))
      .format();
    ...
    for (var i = 0; i < sessions.length; i++) {
      ...
      if (dayjs(lastEnd).format("X") < dayjs(sessions[i].start).format("X")) {
        ...
        const duration =
          dayjs(session.start).format("X") - 1 - dayjs(lastEnd).format("X");
        let label = "";
        let start = "";
        let end = "";
        if (lastEnd == dayStart) {
      ...
      if (
        dayjs(sessions[i].start).format("X") <
        dayjs(data.window_start).format("X")
      ) {
        duration_offset =
          dayjs(data.window_start).format("X") -
          dayjs(sessions[i].start).format("X");
      ...
      if (
        dayjs(sessions[i].end).format("X") > dayjs(data.window_end).format("X")
      ) {
        duration_offset =
          dayjs(sessions[i].end).format("X") -
          dayjs(data.window_end).format("X");
      }
    ...
    if (
      dayjs(pieSessions[0].start).format("X") >
      dayjs(data.window_start).format("X")
    ) {
      let session = pieSessions[0];
      let duration =
        dayjs(session.start).format("X") -
        1 -
        dayjs(data.window_start).format("X");
    ...
    if (
      dayjs(pieSessions[pieSessions.length - 1].end).format("X") <
      dayjs(dayEnd).format("X")
    ) {
    ...
    if (
      ...
      let duration =
        dayjs(data.window_end).format("X") -
        dayjs(session.end).add(1, "minute").format("X");
      duration = duration / 6;
      values.push(duration);
      labels.push(
        dayjs(session.end).add(1, "minute").format("HH:mm") +
