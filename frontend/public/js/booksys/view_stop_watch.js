class BooksysViewStopWatch {
    ...
    if (typeof Storage !== "undefined") {
      ...
      if (localStorage.getItem("watch.running")) {
        ...
        this.watch.startTime = moment(
          localStorage.getItem("watch.startTime"),
          "X"
        );
        this.watch.timeOffset = Number(
          localStorage.getItem("watch.timeOffset")
        );
        ...
        if (localStorage.getItem("watch.isPaused")) {
          ...
          this.watch.pauseTime = moment(
            localStorage.getItem("watch.pauseTime"),
            "X"
          );
        } else {
      ...
      if (localStorage.getItem("watch.heats")) {
        this.heats = new Map(
          Object.entries(JSON.parse(localStorage.getItem("watch.heats")))
        );
      ...
      alert(
        "Watch will not work properly with this browser's setting. Local Storage is not available."
      );
    }
  }
  ...
  resume() {
    this.watch.timeOffset +=
      Number(moment().format("X")) - Number(this.watch.pauseTime.format("X"));
    this.watch.isPaused = false;
  ...
  finish(cb) {
    ...
    localStorage.setItem(
      "watch.heats",
      JSON.stringify(Object.fromEntries(this.heats))
    );
    this.addHeats(cb);
    this.cancel();
  }
  ...
  getDuration() {
    ...
    let timediff =
      this.watch.currentTime.format("X") -
      this.watch.startTime.format("X") -
      this.watch.timeOffset;
    return timediff;
  }
      ...
      success: function (data) {
        ...
        for (let key in response) {
          if (response[key].ok == true) {
            ...
            localStorage.setItem(
              "watch.heats",
              JSON.stringify(Object.fromEntries(that.heats))
            );
          } else {
